...
class YoloDetect():
    def __init__(self, detect_class="person", frame_width=640, frame_height=480):
        # Parameters
    ...
    def alert(self, img):
        ...
        # New thread to send telegram after 10 seconds
    ...
    def detect(self, frame, points, instru_check):
        ...
        for i in indices:
            ...
            w = box[2]
            h = box[3]
            instru_check = instru_check or self.draw_prediction(frame, class_ids[i], round(x), round(y), round(x + w), round(y + h), points)

        return frame, instru_check